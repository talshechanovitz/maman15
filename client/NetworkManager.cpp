...
NetworkManager::NetworkManager() : _ioContext(nullptr), _resolver(nullptr), _socket(nullptr), _connected(false)
{
...
bool NetworkManager::setSocketInfo(const std::string& address, const std::string& port)
{
 ...
 */
bool NetworkManager::isValidAddress(const std::string& address)
{
 ...
 */
bool NetworkManager::isValidPort(const std::string& port)
{
    try
    {
 ...
 */
bool NetworkManager::receive(uint8_t* const buffer, const size_t size) const
{
 ...
 */
bool NetworkManager::send(const uint8_t* const buffer, const size_t size) const
{
    if (_socket == nullptr || !_connected || buffer == nullptr || size == 0)
        return false;
 ...
 */
bool NetworkManager::sendReceive(const uint8_t* const toSend, const size_t size, uint8_t* const response, const size_t resSize)
{
    if (!connect())
    {
        return false;
 ...
 * Handle Endianness.
 */
void NetworkManager::swapBytes(uint8_t* const buffer, size_t size) const
{
